...
| `CommunityToolkit.Mvvm` 提供 `SetProperty(ref _, value, validator)` overload | **确认（错误）**。`ObservableObject.SetProperty<T>(ref T field, T newValue, Func<T, T, bool> validator)` 在 **CT.Mvvm 8.5+** 引入（不是 8.4+）。项目当前版本 = 8.4.2 (`Directory.Packages.props:7`) **不**支持此 overload。**v1.6.1 PATCH 实施时已 pivot 为 manual property + EqualityComparer + OnPropertyChanged pattern**（commit `5279635`），保留 DRY 的 `IsValidRange` 静态 helper。**Erratum**: 后续 spec/plan 不要再假设 8.4+ 有此 overload。 |
