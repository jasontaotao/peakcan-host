...
2. `cd D:/claude_proj2/peakcan-host && git log --oneline` to confirm baseline (HEAD `7c74e5c`).
3. Continue at **Task 13** (App layer — Trace view). From here on, ALL App-layer tasks run serially because they share AppShell.xaml + DI registrations.
...
6. Current test count: **202 pass + 4 SKIP** (Core 137 + Infrastructure 53 + App 12). Test project `tests/PeakCan.Host.App.Tests/` exists and targets `net10.0-windows` with `<UseWPF>true</UseWPF>` (required to instantiate WPF Window types in tests).
