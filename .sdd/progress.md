...
| 14 Send view        | complete | `ce64e3f` feat | 230/230 pass + 5 SKIP (Core 137 + Infrastructure 53 + App 40 — +16 new App tests this task: SendServiceTests 5 + SendViewModelTests 10 + AppShellViewModelTests +1 hardware SKIP); build 0 warn 0 err across 6 projects; pending reviewer |
...
6. Current test count: **214 pass + 4 SKIP** (Core 137 + Infrastructure 53 + App 24). Test project `tests/PeakCan.Host.App.Tests/` exists and targets `net10.0-windows` with `<UseWPF>true</UseWPF>` (required to instantiate WPF Window types in tests).
