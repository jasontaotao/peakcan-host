#!/usr/bin/env python3
"""Tier 3 ship for v3.16.1 PATCH — XAML WatchedSignals binding fix."""
import base64
import json
import subprocess
import sys
...
PARENT_SHA = "5999dc5fe2bf61d1f5a34189a8a40a60c84b4f38"  # v3.16.0 MINOR ship
...
def main():
    ...
    commit_msg = "v3.16.1 PATCH: fix XAML ItemsSource binding (Signals → WatchedSignals) so Add-to-watch entries are visible"
    ...
    tag_result = gh("POST", "git/tags",
        {"tag": "v3.16.1", "message": commit_msg, "object": new_commit_sha, "type": "commit"})
    tag_sha = tag_result["sha"]
    print(f"  tag    {tag_sha}  v3.16.1")

    existing = gh("GET", "git/refs/tags/v3.16.1")
    if existing:
        gh("PATCH", "git/refs/tags/v3.16.1", {"sha": tag_sha, "force": True})
        print(f"  refs/tags/v3.16.1 -> {tag_sha} (force)")
    else:
        gh("POST", "git/refs", {"ref": "refs/tags/v3.16.1", "sha": tag_sha})
        print(f"  refs/tags/v3.16.1 -> {tag_sha}")

    release_notes = (REPO_ROOT / "docs" / "release-notes-v3.16.1.md").read_text(encoding="utf-8")
    release_result = gh("POST", "releases",
        {"tag_name": "v3.16.1", "name": "v3.16.1 PATCH: fix XAML ItemsSource binding (visible Add-to-watch)",
