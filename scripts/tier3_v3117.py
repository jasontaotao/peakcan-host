#!/usr/bin/env python3
"""Tier 3 ship for v3.11.7 PATCH — MultiBinding Mode=OneWay (XamlParseException root-cause fix).

Pattern: 1-commit overlay on the v3.11.6 force-updated parent.

- Parent SHA = 3cffef53900d0f5b280d88c8e9f342ab8d5b10fc (v3.11.6 force-updated on origin/main)
- Overlay = 1 commit from feature/v3-11-1-patch (local 2590b54)
  - 2 files: TraceViewerView.xaml + TraceViewerMasterRadioSmokeTests.cs
- Result: new commit on origin/main + new tag v3.11.7 + new GitHub release
"""
import base64
import json
import subprocess
...
PARENT_SHA = "3cffef53900d0f5b280d88c8e9f342ab8d5b10fc"  # v3.11.6 force-updated on origin/main

REPO_ROOT = Path(r"D:\claude_proj2\peakcan-host")

ADDED_OR_MODIFIED = [
    # M8: MultiBinding Mode=OneWay fix
    "src/PeakCan.Host.App/Views/TraceViewerView.xaml",
    "tests/PeakCan.Host.App.Tests/Views/TraceViewerMasterRadioSmokeTests.cs",
    # Release notes
    "docs/release-notes-v3.11.7.md",
]
...
def main():
    ...
    commit_msg = "v3.11.7 PATCH: MultiBinding Mode=OneWay prevents ConvertBack crash (M8)"
    ...
    tag_result = gh("POST", "git/tags",
        {"tag": "v3.11.7", "message": commit_msg, "object": new_commit_sha, "type": "commit"})
    tag_sha = tag_result["sha"]
    assert len(tag_sha) == 40
    print(f"  tag    {tag_sha}  v3.11.7")

    # v3.8.8 lesson: tag ref may already exist (idempotent ship). Force-update.
    existing = gh("GET", "git/refs/tags/v3.11.7")
    if existing:
        gh("PATCH", "git/refs/tags/v3.11.7", {"sha": tag_sha, "force": True})
        print(f"  refs/tags/v3.11.7 -> {tag_sha} (force)")
    else:
        gh("POST", "git/refs", {"ref": "refs/tags/v3.11.7", "sha": tag_sha})
        print(f"  refs/tags/v3.11.7 -> {tag_sha}")

    release_notes = (REPO_ROOT / "docs" / "release-notes-v3.11.7.md").read_text(encoding="utf-8")
    release_result = gh("POST", "releases",
        {"tag_name": "v3.11.7", "name": "v3.11.7 PATCH: MultiBinding Mode=OneWay prevents ConvertBack crash (M8)",
