#!/usr/bin/env python3
"""Tier 3 ship for v3.11.1 PATCH — refactor cleanups (H7 + H8).

Pattern: 2-commit overlay on the v3.10.0 force-updated parent.

- Parent SHA = 8c26af73a917418cfae8d93d0191dcfd76352659 (v3.10.0 force-updated on GitHub)
- Overlay = 7 entries from feature/v3-11-1-patch (local e4a5806)
- Result: new commit on origin/main + new tag v3.11.1 + new GitHub release
"""
import base64
import json
import subprocess
...
ADDED_OR_MODIFIED = [
    # T4 H8: RebuildSignalsCore split
    ...
    # T2 H7: TraceSessionSnapshotBuilder
...
def main():
    ...
    commit_msg = "v3.11.1 PATCH: refactor cleanups (H7 + H8)"
    ...
    tag_result = gh("POST", "git/tags",
        {"tag": "v3.11.1", "message": commit_msg, "object": new_commit_sha, "type": "commit"})
    tag_sha = tag_result["sha"]
    assert len(tag_sha) == 40
    print(f"  tag    {tag_sha}  v3.11.1")

    # v3.8.8 lesson: tag ref may already exist (idempotent ship). Force-update.
    if gh("GET", "git/refs/tags/v3.11.1"):
        gh("PATCH", "git/refs/tags/v3.11.1", {"sha": tag_sha, "force": True})
        print(f"  refs/tags/v3.11.1 -> {tag_sha} (force)")
    else:
        gh("POST", "git/refs", {"ref": "refs/tags/v3.11.1", "sha": tag_sha})
        print(f"  refs/tags/v3.11.1 -> {tag_sha}")

    release_notes = (REPO_ROOT / "docs" / "release-notes-v3.11.1.md").read_text(encoding="utf-8")
    release_result = gh("POST", "releases",
        {"tag_name": "v3.11.1", "name": "v3.11.1 PATCH: refactor cleanups (H7 + H8)",
