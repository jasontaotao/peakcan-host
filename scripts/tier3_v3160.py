#!/usr/bin/env python3
"""Tier 3 ship for v3.16.0 MINOR — DbcTreePickerWindow."""
import base64
import json
import subprocess
import sys
...
PARENT_SHA = "6953a3f2c8e1a4893519ce07260029672ef43786"  # v3.15.0 MINOR ship

REPO_ROOT = Path(r"D:\claude_proj2\peakcan-host")

ADDED_OR_MODIFIED = [
    "src/PeakCan.Host.App/ViewModels/DbcTreePickerViewModel.cs",
    "src/PeakCan.Host.App/Views/DbcTreePickerWindow.xaml",
    "src/PeakCan.Host.App/Views/DbcTreePickerWindow.xaml.cs",
    ...
    "tests/PeakCan.Host.App.Tests/ViewModels/DbcTreePickerViewModelTests.cs",
...
def main():
    ...
    commit_msg = "v3.16.0 MINOR: DBC tree picker dialog (partial — TabControl + bundle schema v2 deferred to v3.17.0)"
    ...
    tag_result = gh("POST", "git/tags",
        {"tag": "v3.16.0", "message": commit_msg, "object": new_commit_sha, "type": "commit"})
    tag_sha = tag_result["sha"]
    print(f"  tag    {tag_sha}  v3.16.0")

    existing = gh("GET", "git/refs/tags/v3.16.0")
    if existing:
        gh("PATCH", "git/refs/tags/v3.16.0", {"sha": tag_sha, "force": True})
        print(f"  refs/tags/v3.16.0 -> {tag_sha} (force)")
    else:
        gh("POST", "git/refs", {"ref": "refs/tags/v3.16.0", "sha": tag_sha})
        print(f"  refs/tags/v3.16.0 -> {tag_sha}")

    release_notes = (REPO_ROOT / "docs" / "release-notes-v3.16.0.md").read_text(encoding="utf-8")
    release_result = gh("POST", "releases",
        {"tag_name": "v3.16.0", "name": "v3.16.0 MINOR: DBC tree picker (partial)",
