#!/usr/bin/env python3
"""Tier 3 ship for v3.14.2 PATCH — Trace Viewer lazy chart build."""
import base64
import json
import subprocess
import sys
...
PARENT_SHA = "7e9784d805a4b069358f4b53ac00e4a9f1d6e7c5"  # v3.14.1 PATCH ship
...
def main():
    ...
    commit_msg = "v3.14.2 PATCH: Trace Viewer lazy chart build — 5x faster load, plot on demand"
    ...
    tag_result = gh("POST", "git/tags",
        {"tag": "v3.14.2", "message": commit_msg, "object": new_commit_sha, "type": "commit"})
    tag_sha = tag_result["sha"]
    print(f"  tag    {tag_sha}  v3.14.2")

    existing = gh("GET", "git/refs/tags/v3.14.2")
    if existing:
        gh("PATCH", "git/refs/tags/v3.14.2", {"sha": tag_sha, "force": True})
        print(f"  refs/tags/v3.14.2 -> {tag_sha} (force)")
    else:
        gh("POST", "git/refs", {"ref": "refs/tags/v3.14.2", "sha": tag_sha})
        print(f"  refs/tags/v3.14.2 -> {tag_sha}")

    release_notes = (REPO_ROOT / "docs" / "release-notes-v3.14.2.md").read_text(encoding="utf-8")
    release_result = gh("POST", "releases",
        {"tag_name": "v3.14.2", "name": "v3.14.2 PATCH: Trace Viewer lazy chart build (5x faster load)",
