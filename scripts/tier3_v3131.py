#!/usr/bin/env python3
"""Tier 3 ship for v3.13.1 PATCH — TraceSessionRegistry cross-thread CollectionView fix.
...
PARENT_SHA = "e316ca14628dc358887b9efcb75bb401d7a67896"  # v3.13.0 PATCH ship commit on origin/main
...
def main():
    ...
    commit_msg = "v3.13.1 PATCH: TraceSessionRegistry.LoadAsync must resume on UI thread (F4)"
    ...
    tag_result = gh("POST", "git/tags",
        {"tag": "v3.13.1", "message": commit_msg, "object": new_commit_sha, "type": "commit"})
    tag_sha = tag_result["sha"]
    assert len(tag_sha) == 40
    print(f"  tag    {tag_sha}  v3.13.1")

    existing = gh("GET", "git/refs/tags/v3.13.1")
    if existing:
        gh("PATCH", "git/refs/tags/v3.13.1", {"sha": tag_sha, "force": True})
        print(f"  refs/tags/v3.13.1 -> {tag_sha} (force)")
    else:
        gh("POST", "git/refs", {"ref": "refs/tags/v3.13.1", "sha": tag_sha})
        print(f"  refs/tags/v3.13.1 -> {tag_sha}")

    release_notes = (REPO_ROOT / "docs" / "release-notes-v3.13.1.md").read_text(encoding="utf-8")
    release_result = gh("POST", "releases",
        {"tag_name": "v3.13.1", "name": "v3.13.1 PATCH: TraceSessionRegistry cross-thread CollectionView fix",
