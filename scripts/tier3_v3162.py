#!/usr/bin/env python3
"""Tier 3 ship for v3.16.2 PATCH — DbcTreePicker batch-add fix."""
import base64
import json
import subprocess
import sys
...
PARENT_SHA = "b516ae75402c62e4a1446e591b547693819d2a4c"  # v3.16.1 PATCH ship
...
def main():
    ...
    commit_msg = "v3.16.2 PATCH: fix DataGrid ItemContainerGenerator on picker batch add"
    ...
    tag_result = gh("POST", "git/tags",
        {"tag": "v3.16.2", "message": commit_msg, "object": new_commit_sha, "type": "commit"})
    tag_sha = tag_result["sha"]
    print(f"  tag    {tag_sha}  v3.16.2")

    existing = gh("GET", "git/refs/tags/v3.16.2")
    if existing:
        gh("PATCH", "git/refs/tags/v3.16.2", {"sha": tag_sha, "force": True})
        print(f"  refs/tags/v3.16.2 -> {tag_sha} (force)")
    else:
        gh("POST", "git/refs", {"ref": "refs/tags/v3.16.2", "sha": tag_sha})
        print(f"  refs/tags/v3.16.2 -> {tag_sha}")

    release_notes = (REPO_ROOT / "docs" / "release-notes-v3.16.2.md").read_text(encoding="utf-8")
    release_result = gh("POST", "releases",
        {"tag_name": "v3.16.2", "name": "v3.16.2 PATCH: DbcTreePicker batch-add fix (no more DataGrid error)",
