#!/usr/bin/env python3
"""Tier 3 ship for v3.16.6 PATCH — DataGrid cumulative drift + window re-open fix."""
import base64
import json
import subprocess
import sys
...
PARENT_SHA = "7f9224cb1ebaf78e8bd8677af973432b885c6239"  # v3.16.5 PATCH ship
...
def main():
    ...
    commit_msg = "v3.16.6 PATCH: DataGrid cumulative drift fix (FinalizePickerAdds Clear+Re-Add) + window re-open fix (ViewSwitcher cache IsClosed check)"
    ...
    tag_result = gh("POST", "git/tags",
        {"tag": "v3.16.6", "message": commit_msg, "object": new_commit_sha, "type": "commit"})
    tag_sha = tag_result["sha"]
    print(f"  tag    {tag_sha}  v3.16.6")

    existing = gh("GET", "git/refs/tags/v3.16.6")
    if existing:
        gh("PATCH", "git/refs/tags/v3.16.6", {"sha": tag_sha, "force": True})
        print(f"  refs/tags/v3.16.6 -> {tag_sha} (force)")
    else:
        gh("POST", "git/refs", {"ref": "refs/tags/v3.16.6", "sha": tag_sha})
        print(f"  refs/tags/v3.16.6 -> {tag_sha}")

    release_notes = (REPO_ROOT / "docs" / "release-notes-v3.16.6.md").read_text(encoding="utf-8")
    release_result = gh("POST", "releases",
        {"tag_name": "v3.16.6", "name": "v3.16.6 PATCH: DataGrid cumulative drift + window re-open fixes",
