"""W25 T1 deletion script — remove OnChannelFrame (L183-255) from ChannelRouter.cs.
...
W13 T1 2/3 loose-assertion: predicted -73 LoC (range L183-L255 inclusive = 73 lines),
within ±2 LoC tolerance.

Encoding note: ChannelRouter.cs is Windows-1252 (has 0xA1 bytes for non-ASCII
chars like em-dash + middle-dot). Read + write in cp1252.
"""
...
print(f"Expected delta: 73 (Flow C FrameRouting OnChannelFrame range L183-L255). Within ±2 LoC tolerance.")
assert delta == 73 or delta == 72 or delta == 74, f"FAIL: delta {delta} outside ±2 tolerance"
print("PASS.")
