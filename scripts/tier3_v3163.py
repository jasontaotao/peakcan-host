#!/usr/bin/env python3
"""Tier 3 ship for v3.16.3 PATCH — Plot checkbox + playback scrubber fix."""
import base64
import json
import subprocess
import sys
...
PARENT_SHA = "cec5cf8c00644d849edc07a6d2478a37da95b610"  # v3.16.2 PATCH ship
...
def main():
    ...
    commit_msg = "v3.16.3 PATCH: fix ☑ Plot checkbox cast + playback scrubber writeback"
    ...
    tag_result = gh("POST", "git/tags",
        {"tag": "v3.16.3", "message": commit_msg, "object": new_commit_sha, "type": "commit"})
    tag_sha = tag_result["sha"]
    print(f"  tag    {tag_sha}  v3.16.3")

    existing = gh("GET", "git/refs/tags/v3.16.3")
    if existing:
        gh("PATCH", "git/refs/tags/v3.16.3", {"sha": tag_sha, "force": True})
        print(f"  refs/tags/v3.16.3 -> {tag_sha} (force)")
    else:
        gh("POST", "git/refs", {"ref": "refs/tags/v3.16.3", "sha": tag_sha})
        print(f"  refs/tags/v3.16.3 -> {tag_sha}")

    release_notes = (REPO_ROOT / "docs" / "release-notes-v3.16.3.md").read_text(encoding="utf-8")
    release_result = gh("POST", "releases",
        {"tag_name": "v3.16.3", "name": "v3.16.3 PATCH: fix ☑ Plot + playback scrubber",
