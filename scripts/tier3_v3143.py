#!/usr/bin/env python3
"""Tier 3 ship for v3.14.3 PATCH — Trace Viewer DBC-driven signal table + opt-in chart rows."""
import base64
import json
import subprocess
import sys
...
PARENT_SHA = "ada4162ae8d25d45cf5051beaf96380890918d8b"  # v3.14.0 MINOR ship on origin/main (v3.14.1 + v3.14.2 are local-only per PKM capture)

REPO_ROOT = Path(r"D:\claude_proj2\peakcan-host")

ADDED_OR_MODIFIED = [
    "src/PeakCan.Host.App/ViewModels/TraceSignalRow.cs",
...
def main():
    ...
    commit_msg = "v3.14.3 PATCH: Trace Viewer DBC-driven signal table + opt-in chart rows"
    ...
    tag_result = gh("POST", "git/tags",
        {"tag": "v3.14.3", "message": commit_msg, "object": new_commit_sha, "type": "commit"})
    tag_sha = tag_result["sha"]
    print(f"  tag    {tag_sha}  v3.14.3")

    existing = gh("GET", "git/refs/tags/v3.14.3")
    if existing:
        gh("PATCH", "git/refs/tags/v3.14.3", {"sha": tag_sha, "force": True})
        print(f"  refs/tags/v3.14.3 -> {tag_sha} (force)")
    else:
        gh("POST", "git/refs", {"ref": "refs/tags/v3.14.3", "sha": tag_sha})
        print(f"  refs/tags/v3.14.3 -> {tag_sha}")

    release_notes = (REPO_ROOT / "docs" / "release-notes-v3.14.3.md").read_text(encoding="utf-8")
    release_result = gh("POST", "releases",
        {"tag_name": "v3.14.3", "name": "v3.14.3 PATCH: Trace Viewer DBC-driven signal table + opt-in chart rows",
