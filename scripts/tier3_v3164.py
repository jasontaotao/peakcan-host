#!/usr/bin/env python3
"""Tier 3 ship for v3.16.4 PATCH — Multi-agent root-cause fix."""
import base64
import json
import subprocess
import sys
...
PARENT_SHA = "e251d3518100a02a7f62da26858ec13ebd5c710e"  # v3.16.3 PATCH ship
...
def main():
    ...
    commit_msg = "v3.16.4 PATCH: multi-agent root-cause fix — ☑ Plot guard + OxyPlot deferred ItemsSource"
    ...
    tag_result = gh("POST", "git/tags",
        {"tag": "v3.16.4", "message": commit_msg, "object": new_commit_sha, "type": "commit"})
    tag_sha = tag_result["sha"]
    print(f"  tag    {tag_sha}  v3.16.4")

    existing = gh("GET", "git/refs/tags/v3.16.4")
    if existing:
        gh("PATCH", "git/refs/tags/v3.16.4", {"sha": tag_sha, "force": True})
        print(f"  refs/tags/v3.16.4 -> {tag_sha} (force)")
    else:
        gh("POST", "git/refs", {"ref": "refs/tags/v3.16.4", "sha": tag_sha})
        print(f"  refs/tags/v3.16.4 -> {tag_sha}")

    release_notes = (REPO_ROOT / "docs" / "release-notes-v3.16.4.md").read_text(encoding="utf-8")
    release_result = gh("POST", "releases",
        {"tag_name": "v3.16.4", "name": "v3.16.4 PATCH: multi-agent root-cause fix",
