#!/usr/bin/env python3
"""Tier 3 ship for v3.16.7.1 PATCH — fix diagnostic log silence."""
import base64
import json
import subprocess
import sys
...
PARENT_SHA = "0e4bde91b81973e6bf27d206d8910246f68efbfa"  # v3.16.7 PATCH ship
...
ADDED_OR_MODIFIED = [
    ...
    "docs/release-notes-v3.16.7.1.md",
]
...
def main():
    ...
    commit_msg = "v3.16.7.1 PATCH: forward TraceViewerService logger to ReplayTimeline (was NullLogger, swallowed v3.16.7 logs)"
    ...
    tag_result = gh("POST", "git/tags",
        {"tag": "v3.16.7.1", "message": commit_msg, "object": new_commit_sha, "type": "commit"})
    tag_sha = tag_result["sha"]
    print(f"  tag    {tag_sha}  v3.16.7.1")

    existing = gh("GET", "git/refs/tags/v3.16.7.1")
    if existing:
        gh("PATCH", "git/refs/tags/v3.16.7.1", {"sha": tag_sha, "force": True})
        print(f"  refs/tags/v3.16.7.1 -> {tag_sha} (force)")
    else:
        gh("POST", "git/refs", {"ref": "refs/tags/v3.16.7.1", "sha": tag_sha})
        print(f"  refs/tags/v3.16.7.1 -> {tag_sha}")

    release_notes = (REPO_ROOT / "docs" / "release-notes-v3.16.7.1.md").read_text(encoding="utf-8")
    release_result = gh("POST", "releases",
        {"tag_name": "v3.16.7.1", "name": "v3.16.7.1 PATCH: fix diagnostic log silence (logger forwarding)",
