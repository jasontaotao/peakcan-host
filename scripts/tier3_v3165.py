#!/usr/bin/env python3
"""Tier 3 ship for v3.16.5 PATCH — X-axis range overwrite fix."""
import base64
import json
import subprocess
import sys
...
PARENT_SHA = "b823ba65b644708715cd639d026ff5d899da36c4"  # v3.16.4 PATCH ship
...
def main():
    ...
    commit_msg = "v3.16.5 PATCH: X-axis range overwrite fix (4-agent root cause, picker flow)"
    ...
    tag_result = gh("POST", "git/tags",
        {"tag": "v3.16.5", "message": commit_msg, "object": new_commit_sha, "type": "commit"})
    tag_sha = tag_result["sha"]
    print(f"  tag    {tag_sha}  v3.16.5")

    existing = gh("GET", "git/refs/tags/v3.16.5")
    if existing:
        gh("PATCH", "git/refs/tags/v3.16.5", {"sha": tag_sha, "force": True})
        print(f"  refs/tags/v3.16.5 -> {tag_sha} (force)")
    else:
        gh("POST", "git/refs", {"ref": "refs/tags/v3.16.5", "sha": tag_sha})
        print(f"  refs/tags/v3.16.5 -> {tag_sha}")

    release_notes = (REPO_ROOT / "docs" / "release-notes-v3.16.5.md").read_text(encoding="utf-8")
    release_result = gh("POST", "releases",
        {"tag_name": "v3.16.5", "name": "v3.16.5 PATCH: X-axis range overwrite fix",
