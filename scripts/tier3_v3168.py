#!/usr/bin/env python3
"""Tier 3 ship for v3.16.8 PATCH — smoke test + console log."""
import base64
import json
import subprocess
import sys
...
PARENT_SHA = "da08f34a60ce96c633d866a660006176d29ec9ee"  # v3.16.7.1
...
def main():
    ...
    commit_msg = "v3.16.8 PATCH: smoke test log (Console.WriteLine + Serilog) to verify log pipeline is wired"
    ...
    tag_result = gh("POST", "git/tags",
        {"tag": "v3.16.8", "message": commit_msg, "object": new_commit_sha, "type": "commit"})
    tag_sha = tag_result["sha"]
    print(f"  tag    {tag_sha}  v3.16.8")

    existing = gh("GET", "git/refs/tags/v3.16.8")
    if existing:
        gh("PATCH", "git/refs/tags/v3.16.8", {"sha": tag_sha, "force": True})
        print(f"  refs/tags/v3.16.8 -> {tag_sha} (force)")
    else:
        gh("POST", "git/refs", {"ref": "refs/tags/v3.16.8", "sha": tag_sha})
        print(f"  refs/tags/v3.16.8 -> {tag_sha}")

    release_notes = (REPO_ROOT / "docs" / "release-notes-v3.16.8.md").read_text(encoding="utf-8")
    release_result = gh("POST", "releases",
        {"tag_name": "v3.16.8", "name": "v3.16.8 PATCH: smoke test + console log fallback",
