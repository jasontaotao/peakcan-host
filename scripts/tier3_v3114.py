#!/usr/bin/env python3
"""Tier 3 ship for v3.11.4 PATCH — Trace Viewer "Add trace…" empty-path regression fix.

Pattern: 1-commit overlay on the v3.11.3 force-updated parent.

- Parent SHA = e7b72f21a027877459e0956ccb4d3a6b8d74ce24 (v3.11.3 force-updated on origin/main)
- Overlay = 1 commit from feature/v3-11-1-patch (local a22f99f)
  - 3 files: TraceViewerView.xaml + TraceViewerViewModel.cs + TraceViewerViewModelTests.cs
- Result: new commit on origin/main + new tag v3.11.4 + new GitHub release
"""
import base64
import json
import subprocess
...
PARENT_SHA = "e7b72f21a027877459e0956ccb4d3a6b8d74ce24"  # v3.11.3 force-updated on origin/main
...
def main():
    ...
    commit_msg = "v3.11.4 PATCH: Trace Viewer Add trace file-dialog moved to VM (M5)"
    ...
    tag_result = gh("POST", "git/tags",
        {"tag": "v3.11.4", "message": commit_msg, "object": new_commit_sha, "type": "commit"})
    tag_sha = tag_result["sha"]
    assert len(tag_sha) == 40
    print(f"  tag    {tag_sha}  v3.11.4")

    # v3.8.8 lesson: tag ref may already exist (idempotent ship). Force-update.
    existing = gh("GET", "git/refs/tags/v3.11.4")
    if existing:
        gh("PATCH", "git/refs/tags/v3.11.4", {"sha": tag_sha, "force": True})
        print(f"  refs/tags/v3.11.4 -> {tag_sha} (force)")
    else:
        gh("POST", "git/refs", {"ref": "refs/tags/v3.11.4", "sha": tag_sha})
        print(f"  refs/tags/v3.11.4 -> {tag_sha}")

    release_notes = (REPO_ROOT / "docs" / "release-notes-v3.11.4.md").read_text(encoding="utf-8")
    release_result = gh("POST", "releases",
        {"tag_name": "v3.11.4", "name": "v3.11.4 PATCH: Trace Viewer Add trace file-dialog moved to VM (M5)",
