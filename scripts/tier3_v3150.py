#!/usr/bin/env python3
"""Tier 3 ship for v3.15.0 MINOR — Trace Viewer watch list + DBC path."""
import base64
import json
import subprocess
import sys
...
PARENT_SHA = "2d92e260e4e0e1f3ea4d6cffac31f20a60a40c42"  # v3.14.3 PATCH ship on origin/main
...
def main():
    ...
    commit_msg = "v3.15.0 MINOR: Trace Viewer watch list + DBC path tracking (partial — C-1 + bundle deferred to v3.16.0)"
    ...
    tag_result = gh("POST", "git/tags",
        {"tag": "v3.15.0", "message": commit_msg, "object": new_commit_sha, "type": "commit"})
    tag_sha = tag_result["sha"]
    print(f"  tag    {tag_sha}  v3.15.0")

    existing = gh("GET", "git/refs/tags/v3.15.0")
    if existing:
        gh("PATCH", "git/refs/tags/v3.15.0", {"sha": tag_sha, "force": True})
        print(f"  refs/tags/v3.15.0 -> {tag_sha} (force)")
    else:
        gh("POST", "git/refs", {"ref": "refs/tags/v3.15.0", "sha": tag_sha})
        print(f"  refs/tags/v3.15.0 -> {tag_sha}")

    release_notes = (REPO_ROOT / "docs" / "release-notes-v3.15.0.md").read_text(encoding="utf-8")
    release_result = gh("POST", "releases",
        {"tag_name": "v3.15.0", "name": "v3.15.0 MINOR: Trace Viewer watch list + DBC path (partial)",
