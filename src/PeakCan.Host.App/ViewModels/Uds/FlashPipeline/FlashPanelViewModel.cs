...
public sealed partial class FlashPanelViewModel : ObservableObject, IUdsPanel, IDisposable
{
    ...
    public IReadOnlyList<string> CrcPresetNames { get; } =
        [.. PeakCan.Host.Core.Uds.FlashPipeline.CrcParameters.PresetNames, "Custom"];

    /// <summary>
    /// AddressingMode values for the per-step ComboBox.
    /// </summary>
    public IReadOnlyList<PeakCan.Host.Core.Uds.FlashPipeline.AddressingMode> AddressingModes { get; } =
        Enum.GetValues<PeakCan.Host.Core.Uds.FlashPipeline.AddressingMode>();

    /// <summary>
    /// CommunicationSubFunction values for the 0x28 step ComboBox.
    /// </summary>
    public IReadOnlyList<PeakCan.Host.Core.Uds.CommunicationSubFunction> CommunicationSubFunctions { get; } =
        Enum.GetValues<PeakCan.Host.Core.Uds.CommunicationSubFunction>();

    /// <summary>
    /// DtcControlSubFunction values for the 0x14 step ComboBox.
    /// </summary>
    public IReadOnlyList<PeakCan.Host.Core.Uds.DtcControlSubFunction> DtcControlSubFunctions { get; } =
        Enum.GetValues<PeakCan.Host.Core.Uds.DtcControlSubFunction>();
    ...
    /// </summary>
    private PeakCan.Host.Core.Uds.FlashPipeline.CrcParameters ResolveCrcParameters(VerifyParams verify)
    {
        var presets = PeakCan.Host.Core.Uds.FlashPipeline.CrcParameters.Presets;
