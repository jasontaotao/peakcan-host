...
public sealed class VerifyParams : INotifyPropertyChanged
{
    ...
    /// </summary>
    public static Func<int, PeakCan.Host.Core.Uds.FlashPipeline.Segment?>? SegmentResolver { get; set; }
    ...
    private PeakCan.Host.Core.Uds.FlashPipeline.CrcParameters _crcParameters = PeakCan.Host.Core.Uds.FlashPipeline.CrcParameters.Crc32;
    public PeakCan.Host.Core.Uds.FlashPipeline.CrcParameters CrcParameters
    {
        get => _crcParameters;
        set
        {
            ...
            if (_selectedCrcPresetIndex >= 0
                && value != PeakCan.Host.Core.Uds.FlashPipeline.CrcParameters.Presets[_selectedCrcPresetIndex])
            {
    ...
    public int SelectedCrcPresetIndex
    {
        get => _selectedCrcPresetIndex == CustomSentinel
            ? PeakCan.Host.Core.Uds.FlashPipeline.CrcParameters.Presets.Count  // Custom → last dropdown item
            : _selectedCrcPresetIndex;
        set
        {
            int mapped = value >= PeakCan.Host.Core.Uds.FlashPipeline.CrcParameters.Presets.Count
                ? CustomSentinel
                : value;
            ...
            Raise(nameof(IsCrcCustom));
            if (mapped >= 0)
                CrcParameters = PeakCan.Host.Core.Uds.FlashPipeline.CrcParameters.Presets[mapped];
        }
    }
...
{
    public PeakCan.Host.Core.Uds.CommunicationSubFunction SubFunction { get; set; } = PeakCan.Host.Core.Uds.CommunicationSubFunction.DisableRxAndTx;
}

/// <summary>Phase 2: DTCControl (0x14) parameters.</summary>
public sealed record DtcControlParams
{
    public PeakCan.Host.Core.Uds.DtcControlSubFunction SubFunction { get; set; } = PeakCan.Host.Core.Uds.DtcControlSubFunction.ClearDTCInformation;
...
public sealed partial class FlashStep : ObservableObject
{
    ...
    /// </summary>
    [ObservableProperty]
    private PeakCan.Host.Core.Uds.FlashPipeline.AddressingMode _addressingMode = PeakCan.Host.Core.Uds.FlashPipeline.AddressingMode.Physical;
