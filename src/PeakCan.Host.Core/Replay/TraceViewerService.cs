...
public sealed class TraceViewerService : ITraceViewerService, IDisposable
{
    ...
    public TraceViewerService(ILogger<TraceViewerService> logger, ReplayOptions options)
    {
        ...
        _logger = logger;
        _options = options;
        // (Removed) v3.16.8.2 SMOKE instrumentation that appended to a hardcoded
        // absolute path + wrote to stdout/Debug/Trace on every construction.
        // Use the logger only.
        _logger.LogInformation("TraceViewerService ctor ENTER; options.MaxFileSizeBytes={Max}",
