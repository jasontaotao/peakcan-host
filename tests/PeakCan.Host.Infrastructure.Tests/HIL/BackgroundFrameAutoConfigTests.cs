...
public class BackgroundFrameAutoConfigTests
{
    ...
    private static async Task<List<CanFrame>> SendFramesAsync(BackgroundFrame frame, int delayMs = 200)
    {
    ...
    public async Task CounterIncrements_0To15ThenWrap()
    {
        ...
        // late-skip 使 10ms 周期实际 ~15ms tick，需更长的窗口保证 ≥16 帧
        var received = await SendFramesAsync(new BackgroundFrame(Id, Zero8, 10, false, counter, null), delayMs: 400);
    ...
    public async Task CounterWrapsAtMax_ToStartValue()
    {
        ...
        var received = await SendFramesAsync(new BackgroundFrame(Id, Zero8, 10, false, counter, null));
    ...
    public async Task ChecksumXor_WritesXorOfCoveredBytes()
    {
        ...
        var received = await SendFramesAsync(new BackgroundFrame(Id, data, 10, false, null, checksum));
    ...
    public async Task CounterAndChecksumCoexist_ChecksumExcludesCounterBits()
    {
        ...
        var checksum = new ChecksumConfig(StartBit: 56, Length: 8, Algorithm: ChecksumAlgorithm.Xor);
        var received = await SendFramesAsync(new BackgroundFrame(Id, Zero8, 10, false, counter, checksum));
    ...
    public async Task NoAutoConfig_DataUnchanged_BackwardCompat()
    {
        ...
        var received = await SendFramesAsync(new BackgroundFrame(Id, original, 10, false));

        received.Should().NotBeEmpty();
        received[^1].Data.Span.ToArray().Should().Equal(original);
    }
